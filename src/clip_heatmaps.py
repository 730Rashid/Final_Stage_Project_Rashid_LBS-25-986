...
class CLIPAttentionRollout:
    ...
    def __init__(self, model, processor, device: str):
        ...
        self.device = device
        self._cache: dict = {}  # path -> (JPEG bytes, stats dict)
    ...
    def _build_stats(self, sim, grid, sim_min, sim_max):
        ...
        # How concentrated is the attention? (entropy-based)
        # Low entropy = concentrated on few patches, high = spread evenly
        p = sim / (sim.sum() + 1e-8)
        entropy = -np.sum(p * np.log(p + 1e-10))
        max_entropy = np.log(len(sim))
        concentration = 1.0 - (entropy / max_entropy)  # 0 = uniform, 1 = single patch
    ...
    def _draw_colour_bar(self, canvas_bgr, img_width):
        ...
        cv2.putText(canvas_bgr, "Low Attention", (bar_left, label_y),
        ...
        cv2.putText(canvas_bgr, high_text, (bar_right - tw, label_y),
                    font, font_scale, (200, 200, 200), font_thick, cv2.LINE_AA)
    ...
    def compute(self, image_path: str) -> bytes:
        ...
        success, buffer = cv2.imencode(
            ".jpg", canvas_bgr, [cv2.IMWRITE_JPEG_QUALITY, 92]
        )
        if not success:
